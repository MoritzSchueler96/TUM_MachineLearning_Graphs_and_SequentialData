...
class CircleGaussiansDataset(Dataset):
    def __init__(self, n_gaussians=6, n_samples=100, radius=3.0, variance=0.3, seed=0):
...
class MoonsDataset(Dataset):
    def __init__(self, n_samples=1200, seed=0):
        ...
        np.random.seed(seed)
        self.X, _ = make_moons(
            n_samples=n_samples, shuffle=True, noise=0.05, random_state=None
        )
...
def plot_density(model, loader=[], batch_size=100, mesh_size=5.0, device="cpu"):
    ...
    with torch.no_grad():
        ...
        for i in range(0, batch_size ** 2, batch_size):
            data = (
                torch.from_numpy(coords_resh[i : i + batch_size, :]).float().to(device)
            )
            log_prob[i : i + batch_size] = (
                model.log_prob(data.to(device)).cpu().detach().numpy()
            )
...
def plot_samples(model, num_samples=500, mesh_size=5.0):
